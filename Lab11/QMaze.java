...
class QMaze
{
   ...
   public void findPath()
   {
      ...
      for (i=0; i<rowcount; i++)
         for (j=0; j<colcount; j++)
            visited[i][j] = false;
      Cell destination = findPath(new Cell(0,0), new Cell(rowcount-1, colcount-1));
      if (destination == null)
   ...
   private Cell findPath(Cell start, Cell goal)
   {
		...
		while (!q.isEmpty())
		{
         ...
         if (feasible(row, col+1))
            store(q, new Cell(row, col+1, current));
	      // try south
         if (feasible(row+1, col))
            store(q, new Cell(row+1, col, current));
         // try west
         if (feasible(row, col-1))
            store(q, new Cell(row, col-1, current));
         // try north
   ...
   private void store(Queue<Cell> q, Cell nextcell)
   {
